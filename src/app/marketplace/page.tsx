...
// After the Batch 11 FK fix, seller_payment_methods is nested under sellers.
// This helper extracts payment method names from the new nested path.
function extractVerifiedNames(row: ProductWithJoins): string[] {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sellerSpm = ((row.sellers as unknown as Record<string, unknown>)
                         ?.seller_payment_methods as Array<{ payment_methods?: { name?: string } }> | undefined) ?? [];
      return sellerSpm
...
async function loadProducts(): Promise<LoadResult> {
      ...
      return {
              products: rows.map((row) => {
                        const verifiedNames = extractVerifiedNames(row);
