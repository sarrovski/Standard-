...
export async function POST(
  ...
  if (contentType.includes("application/json")) {
    return addVideoMedia(request, productId, auth);
  }
...
async function addImageMedia(
  ...
  if (insertError || !data) {
    ...
    return jsonError(500, `product_media_insert failed: ${detail}`, "product_media_insert", {
...
export async function DELETE(
  ...
  let row:
    | { id: string; product_id: string; storage_path: string | null; media_type: string }
    | null = null;
  if (mediaId) {
    const { data } = await auth.adminSupabase
      ...
      .eq("id", mediaId)
      .eq("product_id", productId)
      .maybeSingle<{ id: string; product_id: string; storage_path: string | null; media_type: string }>();
    row = data ?? null;
  } else if (storagePath) {
    const { data } = await auth.adminSupabase
      .from("product_media")
  ...
  // Storage delete via admin client. YouTube link rows do not have storage.
  if (row.storage_path) {
    ...
    if (storageErr) {
      ...
      return jsonError(500, `storage_delete failed: ${storageErr.message}`, "storage_delete", {
